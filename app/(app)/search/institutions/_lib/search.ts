...
const collection = [env.NEXT_PUBLIC_TYPESENSE_COLLECTION_PREFIX, "institution"].join("");

const queryFields = ["name"] as const;
const sortFields = ["name", "startDate", "endDate", "expandedName", "category", "status"] as const;
const facetFields = ["category", "status", "type"] as const;
...
export const SearchFiltersSchema = v.pipe(
	v.object({
		...
		category: v.fallback(v.pipe(v.unknown(), v.transform(ensureArray), v.array(v.string())), []),
		type: v.fallback(v.pipe(v.unknown(), v.transform(ensureArray), v.array(v.string())), []),
...
export async function getSearchResults(
...
): Promise<SearchResults<Institution>> {
	 ...
	 */
	const response = await client.multiSearch.perform<Array<Institution>>(
		{ searches },
		{
			collection,
...
export async function downloadCollection(
	 ...
	 */
	const searchResponse = await client.multiSearch.perform<Array<Institution>>(
		{
