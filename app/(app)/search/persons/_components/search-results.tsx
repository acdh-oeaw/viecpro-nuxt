...
const columns = [
	{ field: "firstName", label: "Vorname", sort: "firstName" },
	{ field: "name", label: "Nachname", sort: "name" },
	{ field: "startDateWritten", label: "Geburtsdatum", sort: "startDate" },
	{ field: "endDateWritten", label: "Sterbedatum", sort: "endDate" },
	{ field: "status", label: "Status", sort: "status" },
] as const satisfies Array<{ field: keyof Person; label: string; sort: SortField }>;
						...
						{searchResults.hits.map((hit) => {
							...
							return (
								<tr key={searchResult.id} className="relative odd:bg-brand-50">
									{columns.map((column) => {
										return (
											<td key={column.field} className="whitespace-nowrap px-4 py-2.5">
												{column.field === "status" ? (
