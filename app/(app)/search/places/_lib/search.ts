...
const collection = [env.NEXT_PUBLIC_TYPESENSE_COLLECTION_PREFIX, "place"].join("");

const queryFields = ["name"] as const;
const sortFields = ["name", "startDate", "endDate", "category", "status"] as const;
...
export async function getSearchResults(
	 ...
	 */
	const response = await client.multiSearch.perform<Array<Place>>(
		{ searches },
		{
			collection,
