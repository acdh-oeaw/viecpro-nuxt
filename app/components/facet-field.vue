...
const facetModel: Ref<Array<string>> = ref(props.selected ?? []);
...
const loading = computed(() => {
	return (
		...
		selectionQueries.value.some((selectionQuery) => {
			return selectionQuery.isFetching;
		})
	);
});
		...
		if (facetResponse.data.value) {
			retArray.push(
				...facetResponse.data.value.counts
					.filter((count) => {
						return !props.selected?.includes(count.value);
					})
					.sort((a, b) => {
...
<template>
	<div class="flex flex-col">
		<h2 class="flex cursor-auto items-center justify-between text-2xl">
		...
		>
			<label :for="`${fieldName}Search`">
				<Search class="mx-2 size-4 shrink-0 text-gray-400" />
			...
			<button v-if="facetSearch" @click="facetSearch = ''">
				...
				<XCircle class="mx-2 size-5 text-gray-400" />
			</button>
		</div>
		<div v-if="!loading">
			<div
			...
			>
				<input
					...
					v-model="facetModel"
					class="ml-1 size-5 cursor-pointer"
