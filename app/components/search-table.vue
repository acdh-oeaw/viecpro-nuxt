...
const getDetailLink = (id: string, entity?: string) => {
	const type = entity ?? route.path.split("/")[2];
	...
	>
		...
		<div class="mx-auto flex size-full max-w-container flex-col p-2 xl:p-0">
			...
			<div class="flex w-full items-center justify-between">
				<SortMenu class="m-2" :collection="collectionName" :cols="sort ?? []" />
				<DownloadMenu
					v-if="data && data.page"
					:all="data.found"
			...
			<Pagination
				...
				:page="data.page"
			/>
			<LoadingBar class="mt-2 h-1" color="#3C5A50" :disabled="!isPlaceholderData && !loading" />
			<div
				v-if="data"
								...
								>
									...
									>
										...
										<span v-else-if="key === 'ampel'" class="m-2 md:m-0 md:mx-auto">
											<Indicator class="size-5" small :status="hit.document.ampel" />
										</span>
										<span
